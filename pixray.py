...
class MakeCutouts(nn.Module):
    def __init__(self, cut_size, cutn, cut_pow=1., cuts_zoom=False, skip_affine=False):
        ...
        # augmentations.append(K.RandomResizedCrop(size=(self.cut_size,self.cut_size), scale=(0.10,0.25),  ratio=(0.85,1.2), cropping_mode='resample', p=1.0, return_transform=True))
    ...
    def forward(self, input, spot=None):
        ...
        if self.transforms is not None:
            ...
            #     cutout_count += 1
            if self.cuts_zoom == True:
                batch, transforms = self.augs_zoom(torch.cat(cutouts[:], dim=0))
...
def do_init(args):
    ...
    if args.drawer == 'fft':
        ...
        #test_params = drawer.get_fft_noise(init_tensor=None)
        #test_img = TF.to_pil_image(test_params.cpu())
...
def re_average_z(args, cur_it):
    ...
    # drawer.reapply_from_tensor(TF.to_tensor(cur_z_image).to(device).unsqueeze(0), cur_it)
...
def train(args, cur_it):
    ...
    if cur_it < args.iterations:
        ...
        if apply_overlay(args, cur_it) or args.overlay_every == 1 and cur_it >= 1:
            if cur_anim_index is not None:
            ...
            if args.drawer == "fft":
                ...
                cur_z_image1 = cur_z_image1.resize((gside_X, gside_Y), Image.LANCZOS)
                if overlay_image_rgba:
                    cur_z_image1.paste(overlay_image_rgba, (0, 0), mask=overlay_image_rgba)
                    ...
                    #drawer.reapply_from_tensor(new_tensor * 2 - 1, cur_it)
                    drawer.reapply_from_tensor(new_tensor, cur_it)
                    #_{cur_iteration:03d}
            else:        
